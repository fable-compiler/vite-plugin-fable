...
const packageDir = path.join(repoRoot, "packages/vite-plugin-fable");

const version = (await $`bunx changelog --latest-release --file ${changelog}`.text()).trim();

const packageVersion = (await $`npm info vite-plugin-fable version`.text()).trim();
